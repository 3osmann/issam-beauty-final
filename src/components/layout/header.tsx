...
export default function Header() {
  ...
  try {
    if (settings.nav_links) navLinks = JSON.parse(settings.nav_links);
  } catch {}

  const logoType = settings.logo_type || "text";
  const logoImage = settings.logo_image || "";
  const logoAccent = settings.store_logo_accent || "ISSAM";
  const logoText = settings.store_logo_text || " BEAUTY";
          ...
          <div className="flex items-center justify-between h-16 lg:h-20">
            ...
            <Link href="/" className="relative z-10">
              {logoType === "image" && logoImage ? (
                <Image src={logoImage} alt={settings.store_name || "Issam Beauty"} width={160} height={48} className="h-10 w-auto object-contain" priority />
              ) : (
                <h1 className="text-2xl lg:text-3xl font-serif font-bold tracking-tight">
                  <span className="text-gradient">{logoAccent}</span>
                  <span className="text-slate-900 dark:text-white">{logoText}</span>
                </h1>
              )}
            </Link>
            ...
            <div className="flex items-center gap-2 lg:gap-3">
              <button onClick={() => setSearchOpen(true)} className="p-2.5 rounded-xl hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors">
                <Search className="w-5 h-5" />
              </button>
              <Link href="/wishlist" className="relative p-2.5 rounded-xl hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors hidden sm:block">
                <Heart className="w-5 h-5" />
                {wishlistCount > 0 && <span className="absolute -top-0.5 -right-0.5 w-4 h-4 bg-primary-500 text-white text-[9px] font-bold rounded-full flex items-center justify-center">{wishlistCount}</span>}
              </Link>
              <button onClick={toggleCart} className="relative p-2.5 rounded-xl hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors">
                <ShoppingBag className="w-5 h-5" />
                {cartCount > 0 && <span className="absolute -top-0.5 -right-0.5 w-4 h-4 bg-primary-500 text-white text-[9px] font-bold rounded-full flex items-center justify-center">{cartCount}</span>}
              </button>
